...
namespace SchoolInformationManagement
{
	...
	public partial class Form1 : Form
	{
		...
		public Form1()
		{
			...
			string connectionString =
				"Server=DESKTOP-HQP0AN8\\MSSQLSERVER01;Database=SchoolDB;Trusted_Connection=True;TrustServerCertificate=True;";
	...
	public class UserService : IUserService
	{
		...
		{
			if (id <= 0)
				throw new Exception("No student selected for modification!");
...
SELECT st.StudentId AS Id, st.Name, st.Phone, st.Email,
       CAST(NULL AS DECIMAL(18,2)) AS Salary,
       CAST(NULL AS BIT) AS IsFullTime,
       'Student' AS UserRole,
...
SELECT ts.TeacherId AS Id, ts.Name, ts.Phone, ts.Email,
       ts.Salary,
       CAST(NULL AS BIT) AS IsFullTime,
       'TeachingStaff' AS UserRole,
       su.SubjectId, su.SubjectName
...
SELECT ad.AdminId AS Id, ad.Name, ad.Phone, ad.Email,
       ad.Salary,
       ad.IsFullTime,
       'Administrator' AS UserRole,
       CAST(NULL AS INT) AS SubjectId,
       CAST(NULL AS NVARCHAR(MAX)) AS SubjectName
		...
		public void DeleteUser(int id, string role)
		{
			...
			switch (role)
			{
				case "Student":
					tableName = "Students";
					idColumn = "StudentId";
					relationTable = "StudentSubjects";
					relationColumn = "StudentId";
					break;

				case "TeachingStaff":
					tableName = "TeachingStaffs";
					idColumn = "TeacherId";
					relationTable = "TeacherSubjects";
					relationColumn = "TeacherId";
					break;
